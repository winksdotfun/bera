...
const IBGTPage = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const rewardToken = data.reward_tokens.find((token: { address: string, decimals: number, image: string, name: string, price: number, symbol: string, apr: number }) => token.apr !== undefined);
  ...
  const formatTVL = (tvl: number | undefined) => (tvl !== undefined ? (tvl / 1_000_000).toFixed(2) + "M" : "Loading...");
